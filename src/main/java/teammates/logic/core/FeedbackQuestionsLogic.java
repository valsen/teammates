...
public final class FeedbackQuestionsLogic {
    ...
    public Map<String, String> getRecipientsForQuestion(
            ...
            throws EntityDoesNotExistException {
        Diy diy = new Diy();
        diy.initializeFile("getRecipientsForQuestion", 24);
        ...
        case STUDENTS:
            ...
            for (StudentAttributes student : studentsInCourse) {
                ...
                } else diy.setReachedId("getRecipientsForQuestion", 7);
            }
            break;
        case INSTRUCTORS:
            ...
            for (InstructorAttributes instr : instructorsInCourse) {
                ...
                } else diy.setReachedId("getRecipientsForQuestion", 11);
            }
            break;
        case TEAMS:
            ...
            for (TeamDetailsBundle team : teams) {
                ...
                } else diy.setReachedId("getRecipientsForQuestion", 15);
            }
            break;
        case OWN_TEAM:
            diy.setReachedId("getRecipientsForQuestion", 16);
        ...
        case OWN_TEAM_MEMBERS:
            ...
            for (StudentAttributes student : students) {
                ...
                if (!student.email.equals(giver)) {
                    ...
                    recipients.put(student.email, student.name);
                } else diy.setReachedId("getRecipientsForQuestion", 20);
            }
            break;
        case OWN_TEAM_MEMBERS_INCLUDING_SELF:
            diy.setReachedId("getRecipientsForQuestion", 21);
            ...
            for (StudentAttributes student : teamMembers) {
                diy.setReachedId("getRecipientsForQuestion", 22);
                // accepts self feedback too
                recipients.put(student.email, student.name);
            }
            break;
        case NONE:
            diy.setReachedId("getRecipientsForQuestion", 23);
            recipients.put(Const.GENERAL_QUESTION, Const.GENERAL_QUESTION);
            break;
        default:
            diy.setReachedId("getRecipientsForQuestion", 24);
            break;
        }
        return recipients;
    }
    ...
    private Map<String, String> getRecipientsOfQuestion(
        ...
        case STUDENTS:
            ...
            for (StudentAttributes student : studentsInCourse) {
                ...
                if (!giverEmail.equals(student.email)) {
                    ...
                    recipients.put(student.email, student.name);
                } else diy.setReachedId("getRecipientsOfQuestion", 7);
            }
            break;
        case INSTRUCTORS:
            ...
            for (InstructorAttributes instr : instructorsInCourse) {
                ...
                if (!giverEmail.equals(instr.email)) {
                    ...
                    recipients.put(instr.email, instr.name);
                } else diy.setReachedId("getRecipientsOfQuestion", 11);
            }
            break;
        case TEAMS:
            ...
            for (TeamDetailsBundle team : teams) {
                ...
                // Ensure student('s team) does not evaluate own team.
                if (!giverTeam.equals(team.name)) {
                    diy.setReachedId("getRecipientsOfQuestion", 16);
                    // recipientEmail doubles as team name in this case.
                    recipients.put(team.name, team.name);
                } else diy.setReachedId("getRecipientsOfQuestion", 17);
            }
            break;
        case OWN_TEAM:
        ...
        case OWN_TEAM_MEMBERS:
            ...
            List<StudentAttributes> students = studentsLogic.getStudentsForTeam(giverTeam, question.courseId);
            for (StudentAttributes student : students) {
                diy.setReachedId("getRecipientsOfQuestion", 20);
                if (!student.email.equals(giverEmail)) {
                    diy.setReachedId("getRecipientsOfQuestion", 21);
                    recipients.put(student.email, student.name);
                } else diy.setReachedId("getRecipientsOfQuestion", 22);
            }
            break;
        case OWN_TEAM_MEMBERS_INCLUDING_SELF:
            diy.setReachedId("getRecipientsOfQuestion", 23);
            List<StudentAttributes> teamMembers = studentsLogic.getStudentsForTeam(giverTeam, question.courseId);
            for (StudentAttributes student : teamMembers) {
                diy.setReachedId("getRecipientsOfQuestion", 24);
                // accepts self feedback too
                recipients.put(student.email, student.name);
            }
            break;
        case NONE:
            diy.setReachedId("getRecipientsOfQuestion", 25);
            recipients.put(Const.GENERAL_QUESTION, Const.GENERAL_QUESTION);
            break;
        default:
            diy.setReachedId("getRecipientsOfQuestion", 26);
            break;
        }
        return recipients;
    }
